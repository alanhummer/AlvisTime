...
console.log("BACKGROUND LETime: We are running");

var JIRA = JiraAPI(baseUrl, apiExtension, "NoJQLToInitialize");
...
function onUserError(error) {
    console.log("BACKGROUND LETime: Failed to get user:" + error);
    blnPollJira = false;
}
...
function jiraTimeCardApprovalCheck(inputUser) {
        ...
        console.log("BACKGROUND LETime: Polling for submitted/needs approval time cards for " + inputUser.name);
        ...
        .then(function(response) {
            ...
            if (response.issues.length > 0)  {
                console.log("BACKGROUND LETime: We have a submitted card for: " + inputUser.name);
                if (confirm("We have a time card to approve for " + inputUser.name  + " .  Do you want to approve it now?")) {
                    ...
                    console.log("BACKGROUND LETime: Approval found - loaded approval dialogue");               
                }
                else {
                    console.log("BACKGROUND LETime: Approval found - User cancelled reminder dialogue");     
                }
           }
            else {
                console.log("BACKGROUND LETime: We do not have any submitted cards for: " + inputUser.name);
            }
        }, function(error) {
            //Failure
            console.log("BACKGROUND LETime: Failed to get submitted cards for : " + inputUser.name);
        });
}
...
function jiraTimeCardSubmittedCheck() {
    ...
    console.log("BACKGROUND LETime: Polling for completed time cards since: " + startDate);
...
function onFetchSuccess(response) {
    ...
    issues.forEach(function(issue) {
                ...
                if (typeof worklog.comment != "undefined") {
                    ...
                    if (worklog.comment.includes(userId)) {
                        if (worklog.comment.includes("submitted") || worklog.comment.includes("approved")) {
                            ...
                            console.log("BACKGROUND LETime: Found a timecard. All done.");
                            return;
                        }
                    }
                }
                    ...
                    if (issuesProcessed == issues.length) {
                        ...
                        if (blnHaveTimecard) {
                            ...
                            if (confirm("You have not entered time yet for the week.  Deadline is 4PM Friday.  Do you want to enter it now?")) {
                                chrome.windows.create({url: chrome.extension.getURL("popup.html"), type: "popup", height: 639, width:817}); //not sure whey this isnt aboslute, but have to adjust a little to match extenion launch size
                                console.log("BACKGROUND LETime: No timecard found - loaded reminder dialogue");               
                            }
                            else {
                                console.log("BACKGROUND LETime:  No timecard found - User cancelled reminder dialogue");     
                            }
        ...
        function onWorklogFetchError(error) {
            console.log("BACKGROUND LETime: TOOK ERROR LOADING WORKLOG: " + error);
        }
    });
}
...
function onFetchError(error) {
    //Log the error
    console.log("BACKGROUND LETime: TOOK ERROR LOADING ISSUES: " + error);
}
